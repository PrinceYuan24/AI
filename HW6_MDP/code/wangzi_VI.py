...
def student_name():
   return "Wang, Ziyuan" # For an autograder.
...
def one_step_of_VI(S, A, T, R, gamma, Vk):
    ...
    delta_max = 0
    for s in S:
        V_maxValue = -float('inf')
        for a in A:
            Q_value = 0            
            for new_state in S:
                t = T(s, a, new_state)
                r = R(s, a, new_state)
                Q_value = Q_value + t * (r + gamma * Vk[new_state])
            Q_Values_Dict[(s,a)] = Q_value
            if Q_value > V_maxValue:
                V_maxValue = Q_value
        Vkplus1[s] =  V_maxValue
        if delta_max < abs(Vkplus1[s] - Vk[s]):
...
def extract_policy(S, A):
    ...
    Policy = {}
    # Add code here
    Q_Values_dict_cpy = return_Q_values(S,A)
    
    for s in S:
        max_value = -float('inf')
        for a in A:
            if Q_Values_dict_cpy[(s,a)] > max_value:
                max_value = Q_Values_dict_cpy[(s,a)]
                Policy[s] = a
